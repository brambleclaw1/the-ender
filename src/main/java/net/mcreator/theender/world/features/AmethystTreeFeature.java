...
package net.mcreator.theender.world.features;
...
public class AmethystTreeFeature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(ResourceKey.create(Registries.DIMENSION, new ResourceLocation("the_ender:the_ender")));
	private final List<Block> base_blocks;
	private StructureTemplate template = null;

	public AmethystTreeFeature() {
		super(NoneFeatureConfiguration.CODEC);
		base_blocks = List.of(Blocks.END_STONE, Blocks.AMETHYST_BLOCK);
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("the_ender", "amethysttree"));
		if (template == null)
			return false;
